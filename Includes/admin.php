...
protected static $db_fields = array('A_ID','First_name','Last_name','Password','Email');
public $A_ID;
public $First_name;
public $Last_name;
public $Password;
...
public function full_name() {
	if(isset($this->First_name) && isset($this->Fast_name)){
		return $this->First_name.' '.$this->Fast_name;
	}
}
//Common database function
public static function find_all(){
	global $database;
	return self::find_by_sql("SELECT * FROM $tablename");
}
public static function find_by_id($id=0){
	global $database;
	$result_array = self::find_by_sql("SELECT * FROM admin WHERE A_ID ={$id} LIMIT 1");
	return !empty($result_array) ? array_shift($result_array) : false;
}
